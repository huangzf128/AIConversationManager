...
| `chat_messages[].parent_message_uuid` | Parent message UUID; root messages use the sentinel `00000000-0000-4000-8000-000000000000`. Enables branching. |
...
chain to render branch switchers when siblings exist.
...
`type: "thinking"`. The parser extracts these, wraps them in a `thinking`
code fence (same format as DeepSeek), and prepends them to the main
`text` field.
