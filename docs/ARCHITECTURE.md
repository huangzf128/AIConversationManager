...
├── gemini/
│   ├── gemini.importer.ts      # ZIP import (parseZipEntries + parseZipEntriesStreamed)
│   └── gemini.parser.ts        # parse (batch) + parseRecordStream (streaming)
├── utils/
...
| **Gemini** | `createReadStream` ✅ | State machine per chunk ✅ | Streaming per record; messages upserted one by one, conversations batched at end |
| **Claude** | `readFileSync` | Full parse | Files are typically small |

### Gemini Streaming Import

Gemini Takeout JSON is a flat array where each item is a single activity
record (≈ one message), not a complete conversation. The streaming import
(`importFromZipGemini`) works as follows:

1. `splitJsonArrayFile` streams the JSON array, yielding one record at a time
2. Each record is parsed and its `chatId` extracted
3. On first encounter of a `chatId`, the DB is checked for an existing
   conversation (watermark = `dbUpdatedAt`)
4. Messages are upserted one by one; records with `time <= dbUpdatedAt`
   are skipped (watermark filter)
5. Conversation metadata (title, updatedAt) is maintained in memory
6. After all records are processed, conversations are batch-upserted
7. The entire message + conversation write is wrapped in a `$transaction`
8. Attachments are processed after the transaction commits

This avoids `JSON.parse` on the entire file, significantly reducing peak
heap usage for large Takeout exports.
